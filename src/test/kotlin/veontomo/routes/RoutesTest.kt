    ...
    fun testNested() {
        val routerFunction = router {
            ("/api" and accept(MediaType.APPLICATION_JSON) and contentType(MediaType.APPLICATION_JSON)).nest {
                POST("/create", { req ->
                    status(HttpStatus.CREATED).body(bodyFromPublisher(req.bodyToMono(Item::class.java)))
                })
                PUT("/update", { req ->
                    status(HttpStatus.ACCEPTED).body(bodyFromPublisher(req.bodyToMono(Item::class.java)))
                })
            }
        }
